...
numeric shifts, and custom functions. You build a trail by dropping **taps**
into your pipe: transparent pass-throughs that record a snapshot and let the
data flow on. Operation-aware taps like `left_join_tap()` and `filter_tap()` go
further, capturing match rates, drop counts, and stat impact. The result is a
